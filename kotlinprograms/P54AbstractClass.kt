...
/*In Kotlin, abstract class is declared using the abstract
 ...
 instantiated means we can not create object for the
 abstract class.
    Syntax:
...
2.All the variables (properties) and member functions of an
 abstract class are by default non-abstract. So, if we want
  to override these members in the child class then we need
   to use open keyword.
3.If we declare a member function as abstract then we does
 not need to annotate with open keyword because these are
  open by default.
4.An abstract member function doesn’t have a body, and it
...
abstract class AbsBase(val x: String) {   // Non-Abstract Property
    ...
    abstract var y: Int      // explicitly write "abstract" and Abstract Property bydefault open

    abstract fun method1()   // Abstract Methods bydefault open
